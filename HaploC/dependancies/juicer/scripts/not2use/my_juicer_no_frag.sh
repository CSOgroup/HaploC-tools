...
if [[ "$k" == 0 ]]; then
{
    ...
    clumpify.sh -Xmx80g tmpdir=$fastqdir in=${fastq1} in2=${fastq2} out=${name1}.dedup.fastq.gz out2=${name2}.dedup.fastq.gz dedupe subs=5 t=30 2> ${clumpy_logfile}  ## https://www.biostars.org/p/225338/; default is subs=2, but I am more conserved
...
if [[ "$k" == 1 ]]; then
{
...

    {
        ...
        bwa mem -t $threadstring $refSeq ${name1}.dedup.fastq.gz | sort --parallel=20 -S 50G -T $tmpdir -k1,1f > ${name1}.n_sorted.sam
        # rm ${name1}.dedup.fastq.gz

        bwa mem -t $threadstring $refSeq ${name2}.dedup.fastq.gz | sort --parallel=20 -S 50G -T $tmpdir -k1,1f > ${name2}.n_sorted.sam
...
{
    {
        ...
        wait 
        ##########################
        sort --parallel=20 -S 40G -T $tmpdir -k1,1f -m ${name1}.n_sorted.sam ${name2}.n_sorted.sam > ${name}.n_sorted.sam
...
if [[ "$k" == 3 ]]; then
{
    ...
    # sort --parallel=20 -S 40G -T $tmpdir -k2,2d -k6,6d -k4,4n -k8,8n -k1,1n -k5,5n -k3,3n $name.frag.txt > $name.sort.txt
    # echo "Hi I do remove"
    # rm ${name}_norm.txt ${name}.frag.txt

    #MERGE SORTED AND ALIGNED FILES // d: char; n: numeric
    # sort --parallel=20 -S 40G -T $tmpdir -m -k2,2d -k6,6d -k4,4n -k8,8n -k1,1n -k5,5n -k3,3n $splitdir/*.sort.txt  > $outputdir/merged_sort.txt
    sort --parallel=20 -S 40G -T $tmpdir -k2,2d -k6,6d -k3,3n -k1,1n -k5,5n $name.frag.txt  > $outputdir/merged_sort.txt
...
if [[ "$k" == 5 ]]; then
{
    ...
    ${juiceDir}/scripts/common/juicer_tools pre -r 10000,20000,25000,40000,50000,100000,500000,1000000 -s $outputdir/inter.txt -g $outputdir/inter_hists.m -q 1 $outputdir/merged_nodups.txt $outputdir/inter.hic $genomePath
