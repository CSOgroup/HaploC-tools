...
if [[ "$k" == 1 ]]; then
{
...

    {
        ...
        clumpify.sh -Xmx30g tmpdir=$fastqdir in=${fastq1} in2=${fastq2} out=${name1}.dedup.fastq out2=${name2}.dedup.fastq dedupe subs=5 ## https://www.biostars.org/p/225338/; default is subs=2, but I am more conserved
        ...
        bwa mem -t $threadstring $refSeq ${name1}.dedup.fastq | sort --parallel=20 -S 20G -T $tmpdir -k1,1f > ${name1}.n_sorted.sam
        rm ${name1}.dedup.fastq

        bwa mem -t $threadstring $refSeq ${name2}.dedup.fastq | sort --parallel=20 -S 20G -T $tmpdir -k1,1f > ${name2}.n_sorted.sam
        rm ${name2}.dedup.fastq

        echo "Finish bwa & sort at "`date` >> $logfile


        ## no need for the juicer pipline, can be removed
        ...
        wait 
        ##########################
        sort --parallel=20 -S 20G -T $tmpdir -k1,1f -m ${name1}.n_sorted.sam ${name2}.n_sorted.sam > ${name}.n_sorted.sam
        rm ${name1}.n_sorted.sam ${name2}.n_sorted.sam
    }
    echo "Finish STEP_1 at "`date` >> $logfile
...
if [[ "$k" == 2 ]]; then
{
    ...
    sort --parallel=20 -S 20G -T $tmpdir -k2,2d -k6,6d -k4,4n -k8,8n -k1,1n -k5,5n -k3,3n $name.frag.txt > $name.sort.txt
    echo "Hi I do remove"
    rm ${name}_norm.txt ${name}.frag.txt

    #MERGE SORTED AND ALIGNED FILES
    sort --parallel=20 -S 20G -T $tmpdir -m -k2,2d -k6,6d -k4,4n -k8,8n -k1,1n -k5,5n -k3,3n $splitdir/*.sort.txt  > $outputdir/merged_sort.txt
