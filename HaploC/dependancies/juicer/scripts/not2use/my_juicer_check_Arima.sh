...
if [[ "$k" == 0 ]]; then
{
    ...
    clumpify.sh -Xmx60g tmpdir=$fastqdir in=${fastq1} in2=${fastq2} out=${name1}.dedup.fastq.gz out2=${name2}.dedup.fastq.gz dedupe subs=5 t=20 2> ${clumpy_logfile}  ## https://www.biostars.org/p/225338/; default is subs=2, but I am more conserved
...
if [[ "$k" == 1 ]]; then
{
...

    {
        ...
        bwa mem -t $threadstring $refSeq ${name1}.dedup.fastq.gz | sort --parallel=20 -S 40G -T $tmpdir -k1,1f > ${name1}.n_sorted.sam
        # rm ${name1}.dedup.fastq.gz

        bwa mem -t $threadstring $refSeq ${name2}.dedup.fastq.gz | sort --parallel=20 -S 40G -T $tmpdir -k1,1f > ${name2}.n_sorted.sam
