	...
	HaploC()
	{
		...
		k=intg
		log_f=$wk_dir/log_file/log.$k
		jid_intg=$(sbatch -J $k.$line -c 7 --mem 70G --time=1000 --array=1-23 -o $log_f.chr=%a.txt --dependency=afterany:$jid_rep:$jid_snp $job_sh $wk_dir $k | sed 's/Submitted batch job //')
		...
		k=opt
		log_f=$wk_dir/log_file/log.$k.txt
		jid_opt=$(sbatch -J $k.$line -c 23 --mem 100G --time=100 -o $log_f --dependency=afterany:$jid_intg:$jid_sec $job_sh $wk_dir $k | sed 's/Submitted batch job //')
