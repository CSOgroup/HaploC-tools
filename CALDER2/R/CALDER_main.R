    ...
    CALDER = function(contact_tab_dump=NULL, contact_file_dump=NULL, contact_file_hic=NULL, chrs, bin_size, bin_sizes=NULL, save_dir, save_intermediate_data=FALSE, swap_AB=0, genome='others', feature_track=NULL, black_list_region=NULL, n_cores=1, sub_domains=FALSE, single_binsize_only=FALSE)
    {
	        ...
	        {	
	        	bin_size2look = para_tab$bin_size[i]
	           	chr = para_tab$chr[i]
                chr_name = paste0("chr", gsub("chr", "", chr, ignore.case = TRUE))
                ...
                if (!is.null(black_list_region)) black_list_bins = get_black_list_bins(black_list_tab=black_list_region[black_list_region$chr == chr_name, ], bin_size = bin_size2look)[[chr_name]]
